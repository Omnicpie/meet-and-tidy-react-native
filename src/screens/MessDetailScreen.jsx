...
function MessDetailScreen({ navigation, route }) {
  const { id } = route.params;
  const { data, error, loading, refetch } = useQuery(MESS_QUERY, { variables: { id } });
        ...
        <View>
          ...
          <View>
            <Text style={Messes.subheading}>
              Logged on:
              {` ${dayOfMonth(mess.createdAt)} ${shortMonthName(mess.createdAt)} ${fullYear(
                mess.createdAt,
              )}`}
            </Text>
          </View>
