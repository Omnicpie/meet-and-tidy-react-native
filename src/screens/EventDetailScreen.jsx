...
function EventDetailScreen({ navigation, route }) {
  ...
  const [currentUser, setCurrentUser] = useState(store.getState().value);
  ...
  const attending = () =>
    currentUser !== null &&
    data.event.attendances.some(attendance => attendance.userId === currentUser.id);
        ...
        <View>
          <View>
            ...
            <View>
              ...
              <Pressable
                onPress={() =>
                  store.dispatch(setUser({ id: 1, name: 'Beth', email: 'beth@example.com' }))
                }
              >
                <Text>Yo!</Text>
              </Pressable>
